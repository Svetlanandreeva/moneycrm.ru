    ...
    <section style={cardStyle}>
      ...
      {providerState && !providerState.configured && (
        <div style={warningBox}>
          ...
          <p style={{ margin: 0, color: '#707886', fontSize: 8, lineHeight: 1.45 }}>После добавления API-доступа реальные банки включатся автоматически.</p>
        </div>
      )}
        ...
        <div style={connectBox}>
          ...
          {providerState?.configured ? (
            <>
              ...
              <p style={{ margin: '0 0 8px', color: '#59616f', fontSize: 8 }}>
                {providerState.environment === 'sandbox' ? 'Сейчас включена песочница — данные тестовые.' : 'Авторизация откроется на защищённой странице банка.'}
              </p>
              <div style={{ display: 'grid', gap: 6, maxHeight: 260, overflowY: 'auto' }}>
                {providers.map(provider => (
                  <button key={provider.code} disabled={saving} onClick={() => void connect(provider.code)} style={providerButton}>
                    <div style={providerIcon}><Building2 size={13} color="#60a5fa" /></div>
            ...
            <div style={{ display: 'grid', gap: 6 }}>
              {(providerState?.supportedHint ?? ['Сбер', 'Т-Банк', 'Альфа-Банк', 'Точка', 'Модульбанк']).map(name => (
                <div key={name} style={disabledProvider}>
                  <Building2 size={13} /> <span style={{ fontSize: 9, fontWeight: 700 }}>{name}</span>
                  <span style={{ marginLeft: 'auto', fontSize: 7 }}>после активации API</span>
                </div>
              ))}
            </div>
          )}
...
const warningBox = { marginTop: 10, padding: '10px 11px', borderRadius: 11, background: '#f59e0b0d', border: '1px solid #f59e0b28' } as const
...
const disabledProvider = { minHeight: 42, padding: '0 11px', borderRadius: 10, border: '1px solid #20232b', background: '#111318', color: '#626b78', display: 'flex', alignItems: 'center', gap: 8 } as const
