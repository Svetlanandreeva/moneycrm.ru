...
export function BankConnectionsCard({ ctx, onChanged }: { ctx: FinanceContext; onChanged?: () => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('moneycrm:bank-sync-complete', handleComplete)
    return () => window.removeEventListener('moneycrm:bank-sync-complete', handleComplete)
  }, [ctx, onChanged])
  ...
  function requestOzonSync(targetWorkspaceId?: string) {
    setError('')
    setMessage('')
    window.dispatchEvent(new CustomEvent('moneycrm:ozon-sync-request', {
      detail: { workspaceId: targetWorkspaceId || workspaceId || undefined },
    }))
  }
  ...
  async function sync(connection: BankConnection) {
    if (connection.provider === 'ozon_statement') {
      requestOzonSync(connection.workspace_id)
      return
    }
    setSaving(true)
    setError('')
    ...
    <section style={cardStyle}>
      ...
      <div style={safeBox}>
        ...
        <p style={{ margin: 0, color: '#6c7582', fontSize: 8, lineHeight: 1.45 }}>
          На компьютере Ozon синхронизируется одной кнопкой через Chrome Bridge. На телефоне и в PWA уже синхронизированные данные появляются автоматически.
        </p>
      </div>
        ...
        <div style={connectBox}>
          {!fixedKind && (
            <div style={{ marginBottom: 9 }}>
              <p style={fieldLabel}>Куда положить новые счета</p>
          ...
          <p style={{ margin: '0 0 8px', color: '#59616f', fontSize: 8, lineHeight: 1.45 }}>
            Для Ozon: нажмите подключить, выберите дату первой загрузки и войдите в официальный кабинет. Дальше — только «Обновить сейчас».
          </p>
            ...
            {providers.map(provider => (
              <button key={provider.code} disabled={saving} onClick={() => void connect(provider.code)} style={providerButton}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <p style={{ margin: '2px 0 0', color: '#59616f', fontSize: 7, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {provider.code === 'ozon_statement' ? 'Синхронизация через Chrome · без ручного ведения' : provider.description}
                  </p>
                </div>
                {provider.code === 'ozon_statement' ? <RefreshCw size={12} color="#e4f030" /> : <Link2 size={12} color="#59616f" />}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 9, marginTop: 12 }}>
          {visibleConnections.map(connection => {
            ...
            const isOzon = connection.provider === 'ozon_statement'
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {isOzon && <span style={browserBadge}>Chrome sync</span>}
                    </div>
                    <p style={{ margin: '3px 0 0', color: '#59616f', fontSize: 8 }}>{connection.linked_accounts} сч. · обновлено: {dateLabel(connection.last_synced_at)}</p>
                  </div>
                  <div style={{ display: 'flex', gap: 5 }}>
                    <button disabled={saving || (!active && !isOzon)} onClick={() => void sync(connection)} title={isOzon ? 'Обновить Ozon сейчас' : 'Синхронизировать'} style={iconButton}>
                      <RefreshCw size={12} />
                    </button>
                        ...
                        <div key={link.id} style={accountRow}>
                          <div style={{ minWidth: 0, flex: 1 }}>
                            ...
                            <p style={{ margin: '3px 0 0', color: '#59616f', fontSize: 8 }}>
                              {link.account_type === 'credit' ? 'Кредитная карта · долг банку' : link.account_mask ? `•• ${link.account_mask}` : link.external_name || 'Банковский счёт'}
                            </p>
                          </div>
                          <div style={{ width: 128 }}>
...
const browserBadge = { padding: '2px 5px', borderRadius: 6, background: '#e4f03010', border: '1px solid #e4f03025', color: '#cfd86a', fontSize: 7, fontWeight: 800 } as const
