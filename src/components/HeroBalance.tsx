  ...
  return (
    <>
      <div style={{ background: '#e4f030', borderRadius: 24, padding: '24px 20px', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 42, fontWeight: 900, color: '#0c0d10', lineHeight: 1, marginBottom: 20, letterSpacing: '-0.02em' }}>{show ? '1 140 520 ₽' : '••••••• ₽'}</p>
        <div style={{ display: 'flex', background: 'rgba(0,0,0,0.08)', borderRadius: 14, overflow: 'hidden' }}>
          {[['Резервы', '245к'], ['Проекты', '113к'], ['Накопления', '380к']].map(([label, value], i, arr) => (
      ...
      <div style={{ background: '#0f1a0f', border: '2px solid #e4f03030', borderRadius: 20, padding: '18px 20px' }}>
        ...
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 36, fontWeight: 900, color: '#e4f030', lineHeight: 1, marginBottom: 4, letterSpacing: '-0.02em' }}>{show ? '89 200 ₽' : '••••• ₽'}</p>
        <p style={{ fontSize: 11, color: '#4b5563', marginBottom: 12 }}>Счета − резервы − обязательства</p>
        <div style={{ display: 'flex', gap: 12 }}>
          {[['❗ Обязательства', '62 400 ₽'], ['💳 Долги / кредиты', '28 000 ₽']].map(([label, value]) => (
