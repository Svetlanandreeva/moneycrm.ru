...
begin
  ...
  set workspace_id = p_target_workspace_id,
      account_type = case when v_target_kind = 'business' then 'business' else 'bank' end,
      updated_at = now()
...
comment on function public.route_bank_account_context(uuid, uuid) is
'Moves one connected bank account and its non-project transaction history between personal/family/business workspaces. Only the user who connected the bank can route it.';
